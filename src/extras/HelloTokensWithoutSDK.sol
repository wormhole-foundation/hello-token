...
contract HelloTokens is IWormholeReceiver {
        ...
        payable
    {
        ...
        uint64 sequenceA = tokenBridge.transferTokens{value: wormhole.messageFee()}(
        ...
        uint64 sequenceB = tokenBridge.transferTokens{value: wormhole.messageFee()}(
    ...
    function receiveWormholeMessages(
        ...
        ITokenBridge.Transfer memory transferA = tokenBridge.parseTransfer(parsedVMA.payload);
        tokenBridge.completeTransfer(additionalVaas[0]);

        IWormhole.VM memory parsedVMB = wormhole.parseVM(additionalVaas[1]);
        ITokenBridge.Transfer memory transferB = tokenBridge.parseTransfer(parsedVMB.payload);
